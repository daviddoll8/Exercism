...
public class LedgerEntry(string date, string desc, int change)
{
    public DateTime Date { get; } = DateTime.Parse(date, CultureInfo.InvariantCulture);
    public string Desc { get; } = desc;
    public decimal Change { get; } = change / 100.0m;
}

public static class Ledger
{
    public static LedgerEntry CreateEntry(string date, string desc, int change) => new(date, desc, change);
    ...
    private static string PrintEntry(IFormatProvider culture, LedgerEntry entry) =>
        $"{Date(culture, entry.Date)} | {Description(entry.Desc),-25} | {Change(culture, entry.Change),13}";

    private static IEnumerable<LedgerEntry> sort(LedgerEntry[] entries) => entries.OrderBy(x => x.Date + "@" + x.Desc + "@" + x.Change);
